  ...
  spec {
    ...
    template {
      ...
      spec {
        ...
        container {
          name              = "dev"
          image             = local.has_repo && var.cache_repo != "" ? envbuilder_cached_image.cached.0.image : (local.has_repo ? local.devcontainer_builder_image : data.coder_parameter.fallback_image.value)
          ...
          dynamic "env" {
            for_each = nonsensitive(local.has_repo && var.cache_repo != "" ? envbuilder_cached_image.cached.0.env_map : local.envbuilder_env)
            content {
              name  = env.key
              value = env.value
            }
...
resource "coder_agent" "main" {
  ...
  metadata {
    ...
    key          = "8_github_auth"
    value        = local.is_github_repo && data.coder_external_auth.github.access_token != "" ? "authenticated" : (local.is_github_repo ? "not configured" : "not needed")
  }
}
...
resource "coder_metadata" "container_info" {
  ...
  item {
    key   = "workspace image"
    value = local.has_repo && var.cache_repo != "" ? envbuilder_cached_image.cached.0.image : (local.has_repo ? local.devcontainer_builder_image : data.coder_parameter.fallback_image.value)
  }
  item {
    key   = "git url"
