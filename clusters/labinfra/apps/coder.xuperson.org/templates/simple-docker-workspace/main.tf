...
resource "coder_agent" "main" {
  ...
  # These scripts run each time the workspace is started.  
  shutdown_script = <<-EOT
    #!/bin/bash
...
resource "kubernetes_pod" "main" {
  ...
  spec {
    # Docker-in-Docker sidecar container - provides Docker daemon
    container {
      name  = "docker-daemon"
      image = "docker:dind"
      security_context {
...
resource "coder_metadata" "workspace_info" {
  ...
  item {
    key   = "Docker"
    value = "Available via sidecar (tcp://localhost:2375)"
  }
  item {
    key   = "Namespace"
    value = var.namespace
